...
words into exact-tiling continuation pieces ("Type+"). Covered here:

* pure `_cap_chunk_words`: passthrough, exact tiling (incl. odd
  whitespace/unicode), byte-offset tiling, suffixing, degenerate inputs
* cold-router integration: cap enforced, dense indices, zero-gap offsets,
  tails chain across prose continuations but never touch code pieces
...
def test_under_cap_passes_through_untouched():
    ...
    out = _cap_chunk_words(c, 512)
    assert out == [c] and out[0] is c


def test_wordless_chunk_passes_through():
    c = _chunk("   \n\n  ", block_type="Paragraph")
    assert _cap_chunk_words(c, 64) == [c]


def test_single_giant_word_cannot_split_but_terminates():
    c = _chunk("a" * 10000)
    out = _cap_chunk_words(c, 64)
...
def test_pieces_tile_exactly_with_odd_whitespace():
    ...
    out = _cap_chunk_words(c, 2)
    assert len(out) == 4  # 8 words, cap 2
    assert "".join(p["chunk_text"] for p in out) == c["chunk_text"]
    # byte offsets tile the parent span with zero gaps
    assert out[0]["start_offset"] == 10
    assert out[-1]["end_offset"] == c["end_offset"]
    for a, b in zip(out, out[1:]):
        assert a["end_offset"] == b["start_offset"]
    for p in out:
        raw = c["chunk_text"].encode("utf-8")
        base = c["start_offset"]
...
def test_first_piece_keeps_type_continuations_suffixed():
    ...
    out = _cap_chunk_words(c, 4)
    assert [p["block_type"] for p in out] == \
        ["CodeBlock", "CodeBlock+", "CodeBlock+"]
    assert all(len(p["chunk_text"].split()) <= 4 for p in out)
...
def test_unicode_multibyte_offsets():
    ...
    out = _cap_chunk_words(c, 4)
    assert "".join(p["chunk_text"] for p in out) == text
    assert out[0]["start_offset"] == 7
    assert out[-1]["end_offset"] == 7 + len(text.encode("utf-8"))
    for a, b in zip(out, out[1:]):
        assert a["end_offset"] == b["start_offset"]
...
def test_reconstruct_round_trips_split_blocks_byte_identical(tmp_path):
    ...
    try:
        ...
        # separator (pre-existing "~98% fidelity" behaviour), so bodies
        # are compared stripped of trailing whitespace.
        body = f"# Doc\n\n{prose}\n\n```py\n{code}\n```\n\n{items}\n\n{quote}"
        chunks = r.embed_engine._split_into_chunks(body, "doc")
